...
class TextChunker:
    ...
    def process_file(self, file_path, context_window=1, percentile_threshold=95, min_chunk_size=3):
        ...
        sentences = self._load_text(file_path)  # file path or string
    ...
    def _load_text(self, input_: str | Path):
        """Load and tokenize text from a file."""
        match input_:
            case Path():
                with open(input_, encoding="utf-8") as file:
                    text = file.read()
            case str():
                text = input_
    ...
    def _create_chunks(self, sentences, breakpoints):
        ...
        for breakpoint_ in breakpoints:
            chunk = " ".join(sentences[start_idx : breakpoint_ + 1])
            chunks.append(chunk)
            start_idx = breakpoint_ + 1
...
def main():
    ...
    chunks = chunker.process_file(file_path, context_window=1, percentile_threshold=95, min_chunk_size=3)
